##############################################################################
# AWS EKS Deployment - Using Existing IAM Roles
# Deploys EKS Cluster & Node Group using provided IAM role ARNs
##############################################################################

param(
    [string]$Region = "ap-southeast-2",
    [string]$ClusterName = "ai-chatbot-cluster",
    [string]$NodeGroupName = "ai-chatbot-node-group",
    [string]$ProjectName = "ai-chatbot",
    [string]$ClusterRoleArn = "",
    [string]$NodeRoleArn = ""
)

$ErrorActionPreference = "Continue"
$ProgressPreference = "SilentlyContinue"

function Write-OK { Write-Host "[OK] $args" -ForegroundColor Green }
function Write-Error2 { Write-Host "[ERROR] $args" -ForegroundColor Red }
function Write-Info { Write-Host "[INFO] $args" -ForegroundColor Cyan }
function Write-Warn { Write-Host "[WARN] $args" -ForegroundColor Yellow }
function Write-Step { Write-Host "`n>>> $args" -ForegroundColor Yellow }

Write-Host @"
AWS EKS Deployment - Using Existing Roles
Region: $Region | Project: $ProjectName
"@ -ForegroundColor Cyan

# If roles not provided, try to get them
if (-not $ClusterRoleArn) {
    Write-Info "Looking for cluster role: AmazonEKSAutoClusterRole"
    $ClusterRoleArn = aws iam get-role --role-name "AmazonEKSAutoClusterRole" --query Role.Arn --output text 2>$null
}

if (-not $NodeRoleArn) {
    Write-Info "Looking for node role: ai-chatbot-eks-node-group-role"
    $NodeRoleArn = aws iam get-role --role-name "ai-chatbot-eks-node-group-role" --query Role.Arn --output text 2>$null
}

# Verify roles exist
if (-not $ClusterRoleArn -or $ClusterRoleArn -like "*NoSuchEntity*") {
    Write-Error2 "Cluster role not found. Create 'AmazonEKSAutoClusterRole' first"
    exit 1
}

if (-not $NodeRoleArn -or $NodeRoleArn -like "*NoSuchEntity*") {
    Write-Error2 "Node role not found. Create 'ai-chatbot-eks-node-group-role' first"
    Write-Info "Required policies:"
    Write-Info "  - AmazonEKSWorkerNodePolicy"
    Write-Info "  - AmazonEKS_CNI_Policy"
    Write-Info "  - AmazonEC2ContainerRegistryReadOnly"
    exit 1
}

Write-OK "Cluster Role: $ClusterRoleArn"
Write-OK "Node Role: $NodeRoleArn"

# === Get VPC & Subnets ===
Write-Step "Getting VPC and Subnets"

$VPC_ID = aws ec2 describe-vpcs --filters "Name=tag:Name,Values=$ProjectName-vpc" `
    --region $Region --query "Vpcs[0].VpcId" --output text 2>$null

if ($VPC_ID -eq "None") {
    Write-Error2 "VPC not found"
    exit 1
}
Write-OK "VPC: $VPC_ID"

$SUBNETS = aws ec2 describe-subnets --filters "Name=vpc-id,Values=$VPC_ID" `
    --region $Region --query "Subnets[*].SubnetId" --output text 2>$null

if (-not $SUBNETS) {
    Write-Error2 "No subnets found"
    exit 1
}
Write-OK "Subnets: $SUBNETS"

# === Create/Check EKS Cluster ===
Write-Step "Creating/Checking EKS Cluster"

$ClusterStatus = aws eks describe-cluster --name $ClusterName --region $Region `
    --query "cluster.status" --output text 2>$null

if ($ClusterStatus -eq "ACTIVE") {
    Write-OK "Cluster is ACTIVE"
} elseif ($ClusterStatus -and $ClusterStatus -ne "None") {
    Write-Warn "Cluster exists with status: $ClusterStatus"
} else {
    Write-Info "Creating EKS Cluster (takes 5-10 minutes)..."
    
    aws eks create-cluster --name $ClusterName --version 1.28 --role-arn $ClusterRoleArn `
        --resources-vpc-config "subnetIds=$SUBNETS" --region $Region 2>$null | Out-Null
    
    Write-OK "Cluster creation initiated"
    Write-Info "Waiting for ACTIVE status..."
    
    $maxWait = 1200
    $elapsed = 0
    
    while ($elapsed -lt $maxWait) {
        $ClusterStatus = aws eks describe-cluster --name $ClusterName --region $Region `
            --query "cluster.status" --output text 2>$null
        
        if ($ClusterStatus -eq "ACTIVE") {
            Write-OK "Cluster is ACTIVE!"
            break
        }
        
        Write-Host "  [$elapsed/$maxWait s] Status: $ClusterStatus" -ForegroundColor Gray
        Start-Sleep -Seconds 30
        $elapsed += 30
    }
    
    if ($ClusterStatus -ne "ACTIVE") {
        Write-Error2 "Cluster failed to reach ACTIVE status"
        exit 1
    }
}

# === Create/Check Node Group ===
Write-Step "Creating/Checking Node Group"

$NgStatus = aws eks describe-nodegroup --cluster-name $ClusterName --nodegroup-name $NodeGroupName `
    --region $Region --query "nodegroup.status" --output text 2>$null

if ($NgStatus -eq "ACTIVE") {
    Write-OK "Node Group is ACTIVE"
} elseif ($NgStatus -and $NgStatus -ne "None") {
    Write-Warn "Node Group exists with status: $NgStatus"
} else {
    Write-Info "Creating Node Group (takes 5-15 minutes)..."
    
    aws eks create-nodegroup --cluster-name $ClusterName --nodegroup-name $NodeGroupName `
        --subnets $SUBNETS --node-role $NodeRoleArn `
        --scaling-config "minSize=2,maxSize=5,desiredSize=2" `
        --instance-types t3.medium --region $Region 2>$null | Out-Null
    
    Write-OK "Node Group creation initiated"
    Write-Info "Waiting for ACTIVE status..."
    
    $maxWait = 1200
    $elapsed = 0
    
    while ($elapsed -lt $maxWait) {
        $NgStatus = aws eks describe-nodegroup --cluster-name $ClusterName --nodegroup-name $NodeGroupName `
            --region $Region --query "nodegroup.status" --output text 2>$null
        
        if ($NgStatus -eq "ACTIVE") {
            Write-OK "Node Group is ACTIVE!"
            break
        }
        
        Write-Host "  [$elapsed/$maxWait s] Status: $NgStatus" -ForegroundColor Gray
        Start-Sleep -Seconds 30
        $elapsed += 30
    }
    
    if ($NgStatus -ne "ACTIVE") {
        Write-Error2 "Node Group failed to reach ACTIVE status"
        exit 1
    }
}

# === Configure kubectl ===
Write-Step "Configuring kubectl"

Write-Info "Updating kubeconfig..."
aws eks update-kubeconfig --region $Region --name $ClusterName 2>$null | Out-Null

Start-Sleep -Seconds 2

Write-Info "Testing cluster connection..."
$clusterInfo = kubectl cluster-info 2>$null

if ($clusterInfo) {
    Write-OK "kubectl connected successfully"
}

# === Summary ===
Write-Step "DEPLOYMENT COMPLETE"

Write-Host @"
Infrastructure Ready:
  Cluster:       $ClusterName
  Region:        $Region
  Status:        ACTIVE
  Node Group:    ACTIVE
  VPC ID:        $VPC_ID

Next Steps:
  1. Verify nodes: kubectl get nodes -o wide
  2. Build images: .\build-and-push-images.ps1
  3. Deploy apps: .\deploy-to-kubernetes.ps1

Available Commands:
  - Cluster status: aws eks describe-cluster --name $ClusterName --region $Region --query cluster.status
  - Nodes status:   aws eks describe-nodegroup --cluster-name $ClusterName --nodegroup-name $NodeGroupName --region $Region --query nodegroup.status
  - Kubectl nodes:  kubectl get nodes -o wide
  - Kubectl pods:   kubectl get pods -n chatbot

"@ -ForegroundColor Green

Write-OK "Ready to proceed with image building and deployment!"
